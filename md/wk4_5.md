...
> Updated dataset shape: (465312, 69)
...
> Updated dataset shape: (465312, 60)
...
> Updated dataset shape: (465312, 49)
...
- neg_closeprice_flag
    ...
    - handled by: removal
    - notes: 0-value
    > updated dataset shape: (465311, 59)
- neg_livingarea_flag
    - count: 165
    - handled by: removal
    - notes: all 0-values
    > updated dataset shape: (465142, 59)
- neg_dom_flag
    - count: 51
    ...
    > updated dataset shape: (465142, 59) || UNCHANGED
...
- sentinel_coords_flag
    - count: 37
    - handled by: removal
    > updated dataset shape: (465104, 59)
- pos_lon_flag
    - count: 31
    - handled by: converted to null
    > updated dataset shape: (465104, 59) || UNCHANGED
- oos_coords_flag
    - count: 4448
    - handled by: removal
    ...
    > updated dataset shape: (465098, 59)
    ...
    > updated dataset shape: (465086, 59)
    ...
    > updated dataset shape: (465079, 59)

In the end, all 10 flagged columns were dropped which leaves us with:
## final dataset shape: (465039, 49)
> recall starting dataset shape (465312, 84)


# ``LISTINGS`` TRANSFORMATION SUMMARY

> STARTING dataset shape: (631999, 86)
...
> Updated dataset shape: (631999, 73)

The following __14__ columns were considered and removed for having __over 50% nulls__:
- AssociationFeeFrequency
- BuyerAgencyCompensation
- BuyerAgencyCompensationType
- BuyerAgentFirstName
...
> Updated dataset shape: (631999, 59)
...
> Updated dataset shape: (631999, 42)
...
- neg_livingarea_flag
    ...
    - handled by: removal
    - notes: all 0-values
    > updated dataset shape: (631596, 49)
- neg_dom_flag
    - count: 32
    - handled by: converted to null
    ...
    > updated dataset shape: (631596, 49) || UNCHANGED
...
- sentinel_coords_flag
    - count: 75
    - handled by: removal
    > updated dataset shape: (631518, 49)
- pos_lon_flag
    - count: 92
    - handled by: converted to null
    > updated dataset shape: (631518, 49) || UNCHANGED
- oos_coords_flag
    - count: 81007
    - handled by: removal
    - transformation 1: removed non-CA coords via lat/lon
    > updated dataset shape: (631305, 49)
    - transformation 2: removed non-CA coords via postal code
    > updated dataset shape: (631274, 49)
    ...
    > updated dataset shape: (631250, 49)

In the end, all 7 flagged columns were dropped which leaves us with:
## final dataset shape: (631250, 42)
> recall starting dataset shape (631999, 86)
