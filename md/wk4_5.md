...
> Updated dataset shape: (480417, 69)
...
> Updated dataset shape: (480417, 60)

The remaining 60 columns were manually inspected and the following 11 were removed as they were considered to be unnecessary for analysis:
- ListAgentFirstName
- ListAgentLastName
- ListAgentEmail
- StreetNumberNumeric
...
> Updated dataset shape: (480417, 49)
...
- neg_closeprice_flag
    ...
    - handled by: removal
    - notes: 0-value
    > updated dataset shape: (480416, 59)
- neg_livingarea_flag
    - count: 165
    - handled by: removal
    - notes: all 0-values
    > updated dataset shape: (480244, 59)
- neg_dom_flag
    - count: 51
    ...
    > updated dataset shape: (480244, 59) || UNCHANGED
...
- sentinel_coords_flag
    - count: 37
    - handled by: removal
    > updated dataset shape: (480202, 59)
- pos_lon_flag
    - count: 31
    - handled by: converted to null
    > updated dataset shape: (480202, 59) || UNCHANGED
- oos_coords_flag
    - count: 4448
    - handled by: removal
    - transformation 1: null cities were mapped according to zipcode
    > updated dataset shape: (480196, 59) || UNCHANGED
    - transformation 2: out of state coordinates removed
    > updated dataset shape: (480184, 59)
    - transformation 3: 'Other' cities were renamed according to zipcode
    - transformation 4: remove more out of state rows via city label
    > updated dataset shape: (480177, 59)
    - transformation 5: manually fix palmdale, oakland, long beach zipcodes
    - transformation 6: normalize zipcodes to remove hyphens
    - transformation 7: remove out of state rows via zipcode
    > updated dataset shape: (480137, 59)
    - transformation 8: remove specific zipcodes (99999, 97381, 98245, 99228)
    > updated dataset shape: (480134, 59)

In the end, all 10 flagged columns were dropped which leaves us with:
## final dataset shape: (480134, 49)
> recall starting dataset shape (480417, 84)


# ``LISTINGS`` TRANSFORMATION SUMMARY

> STARTING dataset shape: (654108, 86)
...
> Updated dataset shape: (654108, 73)

The following __16__ columns were considered and removed for having __over 50% nulls__:
- AssociationFeeFrequency
- BuyerAgencyCompensation
- BuyerAgencyCompensationType
- BuyerAgentFirstName
...
> Updated dataset shape: (654108, 59)

The remaining 59 columns were manually inspected and the following __17 were removed__ as they were considered to be unnecessary for analysis:
- PropertyType.1
...
> Updated dataset shape: (654108, 42)
...
- neg_livingarea_flag
    ...
    - handled by: removal
    - notes: all 0-values
    > updated dataset shape: (653689, 49)
- neg_dom_flag
    - count: 32
    - handled by: converted to null
    ...
    > updated dataset shape: (653689, 49) || UNCHANGED
...
- sentinel_coords_flag
    - count: 75
    - handled by: removal
    > updated dataset shape: (653603, 49)
- pos_lon_flag
    - count: 92
    - handled by: converted to null
    > updated dataset shape: (653603, 49) || UNCHANGED
- oos_coords_flag
    - count: 81007
    - handled by: removal
    - transformation 1: removed non-CA coords via lat/lon
    > updated dataset shape: (653380, 49)
    - transformation 2: removed non-CA coords via postal code
    > updated dataset shape: (653349, 49)
    - transformation 3: mapped null cities according to zipcode
    - transformation 4: normalized zipcodes to remove hyphens
    - transformation 5: removed rows with 99999, 97381, 98245, 99228 postal codes
    > updated dataset shape: (653323, 49)

In the end, all 7 flagged columns were dropped which leaves us with:
## final dataset shape: (653323, 43)
> recall starting dataset shape (654108, 86)
